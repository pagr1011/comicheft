...
/**
 * Das Modul besteht aus der Klasse {@linkcode ComicheftValidationService}.
...
@Injectable()
export class ComicheftValidationService {
    #ajv = new Ajv2020({
        allowUnionTypes: true,
        allErrors: true,
    });

    readonly #logger = getLogger(ComicheftValidationService.name);
     ...
     */
    validate(comicheft: Comicheft) {
        this.#logger.debug('validate: comicheft=%o', comicheft);
        const validate = this.#ajv.compile<Comicheft>(jsonSchema);
        validate(comicheft);
