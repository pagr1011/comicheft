...
import { entities } from '../comicheft/entity/entities.js';
...
// nullish coalescing
const database = DB_NAME ?? Comicheft.name.toLowerCase();
const { detected } = k8sConfig;

const host = detected ? dbType : DB_HOST ?? 'localhost';
const username = DB_USERNAME ?? Comicheft.name.toLowerCase();
